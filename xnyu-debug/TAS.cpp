...
bool VariableCondition(std::string condition)
{
    ...
    if (paramA.find(".") != std::string::npos && !containsDouble(paramA))
    {
        // Debug address
    ...
    else
    {
        ...
        if (paramC.find(".") != std::string::npos && !containsDouble(paramC))
        {
            // Debug address
...
int PlayInstruction(Instruction instruction)
{
    ...
    else if (instruction.type == "log")
    {
        ...
        for (int i = 0; i < instruction.parameter.size(); i++)
        {
            ...
            {
                if (instruction.parameter[i].find(".") != std::string::npos && !containsDouble(instruction.parameter[i]))
                {
                    // Debug address
    ...
    else if (instruction.type == "byte" || instruction.type == "int32" || instruction.type == "int64" || instruction.type == "float" || instruction.type == "double" || instruction.type == "string" || instruction.type == "bool")
    {
        ...
        if (instruction.parameter[1].find(".") != std::string::npos && !containsDouble(instruction.parameter[1]))
        {
            // Debug address
    ...
    else if (instruction.type == "plus" || instruction.type == "minus" || instruction.type == "multi" || instruction.type == "divide" || instruction.type == "equal")
    {
        ...
        if (instruction.parameter[0].find(".") != std::string::npos && !containsDouble(instruction.parameter[0]))
        {
    ...
    {
        // Function
        if (instruction.type.find(".") != std::string::npos && !containsDouble(instruction.type))
        {
            // Debug function
            ...
            if (FindDebugFunction(parentName, childName, &debugFunction))
            {
                for (int i = 0; i < instruction.parameter.size(); i++)
                {
                    ...
                    if (value.find(".") != std::string::npos && !containsDouble(value))
                    {
                        // Debug address
            ...
            if (TASRecordFrameByFrame)
            {
                ...
                if (TASRecordCanExecute)
                {
                    ...
                    if (!TASRecordScriptUninit)
                    {
                        ...
                        {
                            while (TASSynchronizerFinishedCurrent < TASSynchronizerFinishedMax) SleepExReal(1);
                        }
