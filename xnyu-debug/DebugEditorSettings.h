...
std::vector<FeatureSettingParent> DebugEditorModeSettings;
std::vector<EditorModeActionParent> DebugEditorModeActions;

bool LoadEditorModeSettings()
{
    try
    {
        for (const auto& _file : std::filesystem::directory_iterator(GlobalSettings.config_editormode_settings_directory))
        {
        ...
        for (const auto& _file : std::filesystem::directory_iterator(GlobalSettings.config_editormode_actions_directory))
        {
            std::ifstream address_file = std::ifstream(_file.path());
            std::string fileText((std::istreambuf_iterator<char>(address_file)), std::istreambuf_iterator<char>());
            address_file.close();
            ...
            EditorModeActionParent debugActionParent;
            while (!fileText.empty())
            {
                size_t braceStart = fileText.find("{");
                size_t braceEnd = fileText.find("}");
                ...
                if (fileText[0] != ' ' && fileText[0] != '{')
                {
                    debugActionParent = EditorModeActionParent();
                    ...
                    for (int i = 0; i < childrenSplitted.size() - 1; i++)
                    {
                        ...
                        EditorModeAction debugActionChild = EditorModeAction();
                        nameStart = childrenTarget.find_first_not_of(' ');
                        nameEnd = childrenTarget.find_first_of('(', nameStart) - 1;
