...
HANDLE DebugFunctionsMutex = CreateMutex(NULL, FALSE, NULL);
...
bool LoadDebugFunctions()
{
    try
    {
        WaitForSingleObject(DebugFunctionsMutex, INFINITE);
        std::string debugFunctionsDirectorySetting = GetGlobalSetting("config_debugfunction_directory");
        for (const auto& _file : std::filesystem::directory_iterator(debugFunctionsDirectorySetting))
        {
            ...
            while (!fileText.empty())
            {
                ...
                if (fileText[0] == '{')
                {
                    ReleaseMutex(DebugFunctionsMutex);
                    return false;
                }
